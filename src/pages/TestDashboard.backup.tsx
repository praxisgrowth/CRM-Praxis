          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 glass-panel p-8 rounded-3xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-64 h-64 bg-praxis-cyan/5 blur-[100px] -z-10 group-hover:bg-praxis-cyan/10 transition-all duration-700"></div>
                ...
                <div className="h-80 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={mrrData}>
                      ...
                      <YAxis stroke="#4b5563" fontSize={12} axisLine={false} tickLine={false} tickFormatter={(v) => `R$${v/1000}k`} />
                      <Tooltip 
              ...
              <div className="glass-panel p-8 rounded-3xl">
                ...
                <div className="flex flex-col items-center justify-center relative py-4">
                  <div className="relative w-48 h-24 overflow-hidden">
                    <div className="w-48 h-48 border-[14px] border-white/5 rounded-full"></div>
                    <div 
            ...
            <div className="glass-panel p-8 rounded-3xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
                <ClientTableItem name="TechVision" sub="SaaS - R$ 18k/mês" score={92} status="Excellent" trend="up" />
                <ClientTableItem name="Nexus Corp" sub="Indústria - R$ 14k/mês" score={78} status="Attention" trend="up" />
                <ClientTableItem name="DataFlow" sub="Fintech - R$ 22k/mês" score={65} status="Attention" trend="down" />
                <ClientTableItem name="Retail Max" sub="Varejo - R$ 9k/mês" score={43} status="Critical" trend="down" />
              </div>
            </div>
...
function NavItem({ icon, label, active = false, expanded = true }: { icon: React.ReactNode, label: string, active?: boolean, expanded?: boolean }) {
  return (
    <button className={`
      w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-300 group
      ${active ? 'bg-praxis-cyan/10 border-l-2 border-praxis-cyan text-white' : 'text-gray-500 hover:text-gray-200 hover:bg-white/5 border-l-2 border-transparent'}
    `}>
      <span className={`${active ? 'text-praxis-cyan' : 'group-hover:text-praxis-cyan transition-colors'}`}>{icon}</span>
      {expanded && <span className="text-sm font-medium">{label}</span>}
    </button>
  );
}

function NavGroup({ label, icon, children, expanded = true }: { label: string, icon: React.ReactNode, children: React.ReactNode, expanded?: boolean }) {
...
function KPICard({ title, value, trend, icon }: { title: string, value: string, trend: string, icon: React.ReactNode }) {
  return (
    <div className="glass-panel p-6 rounded-2xl hover:translate-y-[-4px] transition-all duration-300 cursor-default group">
...
function ClientTableItem({ name, sub, score, status, trend }: { name: string, sub: string, score: number, status: string, trend: string }) {
  return (
