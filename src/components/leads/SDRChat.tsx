...
const QUICK_REPLIES = [
  {
    ...
    icon: Zap,
    script: (name: string) =>
      `${name}, que tal agendarmos uma reunião de 30 minutos para eu entender melhor o seu negócio e apresentar como podemos ajudar?`,
  },
  {
    label: 'Enviar Proposta',
    icon: Zap,
    script: (name: string) =>
      `${name}, vou te enviar nossa proposta personalizada em breve. Qualquer dúvida, estou à disposição!`,
  },
  {
    label: 'Pedir Faturamento',
    icon: Zap,
    script: (name: string) =>
      `${name}, para montar a melhor solução para o seu perfil, poderia me informar o faturamento mensal aproximado da sua empresa?`,
  },
]

export function SDRChat({ lead, draft, onDraftChange }: Props) {
      ...
      <div className="p-4 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
        ...
        <div className="flex gap-2">
          {QUICK_REPLIES.map(qr => (
            <button
              key={qr.label}
              onClick={() => setMsg(qr.script(firstName))}
      ...
      <div className="p-4 border-t border-white/5 bg-white/[0.01]">
        <div className="relative group">
          <textarea
            ...
            placeholder={`Responder para ${firstName}...`}
